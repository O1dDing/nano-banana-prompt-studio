...
def capabilities(model: str = "gpt-image-2") -> dict[str, Any]:
    ...
    return {"label": "OpenAI Images", "max_reference_images": 16,
        ...
        "options": {
            "aspect_ratio": select("宽高比", "1:1", list(SIZE_MAP["2K"])),
            "image_size": select("尺寸档位", "2K", list(SIZE_MAP)),
            "size": {"label": "精确尺寸", "type": "text", "default": "",
                     "values": SIZES if flexible_size(model) else SIZES[:4],
                     "placeholder": "留空 = 自动",
                     "help": "16 的倍数；每边≤3840；比例≤3:1；655360～8294400 像素。"},
            "quality": select("生成质量", "auto", quality),
            "output_format": select("输出格式", "png", ["png", "jpeg", "webp"]),
            "background": select("背景", "auto", ["auto", "opaque", "transparent"]),
            "moderation": select("审核强度", "auto", ["auto", "low"]),
            "n": {"label": "每次生成张数", "type": "number", "default": 1, "min": 1, "max": 10, "step": 1},
        }}
